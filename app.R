...
server <- function(input, output) {
  ...
  output$size_info <- renderDataTable({
    ...
    nTrange <- range(table(df()[, input$id_var]))
    nTrange <- paste0("(", round(nTrange[1], 2), ", ", round(nTrange[2], 2), ")")
    ...
    sum_tb <- data.frame(c("Number of participants", 
                         ...
                         c(Nsize, nTrange, Nfreq, Trange, medT))
  ...
  output$time_bar1 <- renderUI({
    ...
    if(input$time_type == "Discrete"){
      sliderTextInput("time_bar1", label = input$time_var, choices = tvec, selected = tvec[1],
                      grid = TRUE)
    }
    else {
      sliderInput("time_bar1", label = input$time_var, 
  ...
  output$heatmap <- renderPlot({
    ...
    ## heatmap
    ht_map <- data.frame(cormat) %>% rownames_to_column("var1") %>%
      pivot_longer(-var1) %>%
    ...
    if(input$time_type == "Discrete"){
      ht_map <- ht_map+
        labs(x="", y="", fill="Correlation", title = "Empirical correlation")
    } else {
      ht_map <- ht_map+
        labs(x="", y="", fill="Correlation", title = "Correlation of smoothed trajectories")
    }
    ht_map
