...
\tikzstyle{io} = [rectangle, rounded corners, text centered, fill = blue!10, text width = 3cm]
\tikzstyle{process} = [rectangle, rounded corners, text centered, text width = 3cm, draw = blue!10]
...
\author{Ying Jin, Shanshan Zhao, David Umbach, Mandy Goldberg}
\date{\today}
...
\label{sec:intro2}
% exisiting tools for multivariate temporal data EDA:
% pairwise
...
\subsubsection{Data upload and preview}
...
\subsubsection{Temporal visualization}
...
\subsubsection{Integrated visualization}
...
The goal is to find a low-dimensional representation that stays true to the high-dimensional representation. Therefore, we would like to minimize the difference between $\mathbf{\Delta}$ and $\mathbf{D}$. This leads to the stress loss function\cite{kruskal1964}: 
...
% \subsubsection{Measurement grid density}
...
% \subsubsection{Measurement grid irregularity}
